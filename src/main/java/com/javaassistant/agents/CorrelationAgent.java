...
public interface CorrelationAgent {
    ...
    @UserMessage("""
            ...
            Rules:
            1. Synthesize the incident from the per-artifact bounded contexts and specialist observations.
            2. If artifact coverage indicates omitted detail or you need more detail from a particular artifact to resolve a contradiction or verify a hypothesis, you must retrieve more context instead of guessing. A blank selector returns the next omitted slice for that artifact. Use sliceId=<id> to reopen a specific slice, page long slices with sliceId=<id>, offset=<charOffset>, chars=<charCount>, and if a retrieval result says More available: true for relevant context, continue expanding until the uncertainty is resolved or the tool budget is exhausted.
