...
public interface PmapAgent {
    ...
    @SystemMessage("""
        ...
        You analyze pmap output using a bounded starting context built from structured summaries, highlighted evidence, and representative source excerpts, with baseline and current sections when a comparison is available.
    ...
    @UserMessage("""
            ...
            4. Explain the most likely native-memory shape, strongest evidence, uncertainty or missing context, and the best next actions.
            5. Do not invent mapping growth or memory totals that are not supported by the diagnostic data.
            6. If the tool budget is exhausted and uncertainty remains, say so clearly.
            7. Never refer to the diagnostic data as a packet, payload, prompt, parser output, assessor output, evidence anchors, traceability, or supervisor trace. Refer directly to the pmap output or comparison instead.
