...
public class NMTTools {
    ...
    @Tool("Fetch more Native Memory Tracking context from the current artifact. Leave selectorQuery blank to get the next omitted slice. Use sliceId=categoryDeltas to reopen a specific slice, and page long slices with sliceId=<id>, offset=<charOffset>, chars=<charCount>. Other selectors include section=categories, category=Class, category=Thread, lines=10-50, pattern=Metadata, or section=metaspaceSummary. For compare, artifactRef can be baseline or current.")
    ...
    @Tool("Compute a focused NMT view from the current artifact. Good requests include metaspace-summary, thread-summary, category-summary, or delta-summary.")
