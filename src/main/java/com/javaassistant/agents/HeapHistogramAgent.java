...
public interface HeapHistogramAgent {
    ...
    @SystemMessage("""
        ...
        You analyze heap histograms using a bounded starting context built from structured summaries, top-class breakdowns, highlighted evidence, and representative source excerpts, with comparison or sequence sections when they are available.
    ...
    @UserMessage("""
            ...
            5. Explain the most likely heap-retention problem, strongest evidence, uncertainty or missing context, and the best next actions.
            6. Do not invent class growth, object counts, or leak claims that are not supported by the diagnostic data.
            7. If the tool budget is exhausted and uncertainty remains, say so clearly.
            8. Never refer to the diagnostic data as a packet, payload, prompt, parser output, assessor output, evidence anchors, traceability, or supervisor trace. Refer directly to the heap histogram, comparison, or sequence instead.
    ...
    @UserMessage("""
            Use the following heap-histogram diagnostic context to answer the user's follow-up question:
            {{diagnosticContext}}

            User question: {{question}}

            The diagnostic context above is intentionally bounded. Additional curated retrieval and focused computation tools are available if you need more detail from the same diagnostic artifact, comparison, or sequence.
