    ...
    public void closeQuietly() {
        ...
        if (chatModel instanceof AutoCloseable autoCloseable) {
            try {
                ...
                // Best-effort cleanup for CLI shutdown.
            }
            return;
        }

        try {
            Method closeMethod = chatModel.getClass().getMethod("close");
            if (closeMethod.getParameterCount() == 0) {
                closeMethod.invoke(chatModel);
            }
        } catch (ReflectiveOperationException ignored) {
            // Best-effort cleanup for providers that do not expose a close hook.
        }
    }
